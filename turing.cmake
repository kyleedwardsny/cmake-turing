...
foreach(line IN LISTS turing_descriptor_contents)
  # Simultaneously parse and check the syntax of each line
  if(NOT line MATCHES "^([a-zA-Z0-9_-]+) (0|1) (LEFT|RIGHT) ([a-zA-Z0-9_-]+) (0|1) (LEFT|RIGHT) ([a-zA-Z0-9_-]+)$")
    message(FATAL_ERROR "Syntax error in ${TURING_DESCRIPTOR_FILE} line ${linenum}: \"${line}\"")
  endif()

  # Put the contents into variables
  set(state_name "${CMAKE_MATCH_1}")
  set(write_value_0 "${CMAKE_MATCH_2}")
  set(next_direction_0 "${CMAKE_MATCH_3}")
  set(next_state_0 "${CMAKE_MATCH_4}")
  set(write_value_1 "${CMAKE_MATCH_5}")
  set(next_direction_1 "${CMAKE_MATCH_6}")
  set(next_state_1 "${CMAKE_MATCH_7}")
...
if(NOT turing_input_contents MATCHES "^([01]*)\n?$")
...
while(NOT current_state MATCHES "^(ACCEPT|REJECT)$")
  ...
  if(next_direction STREQUAL "LEFT")
    if(tape_position EQUAL 0)
      ...
      set(tape_contents "0${tape_contents}")
    else()
    ...
    if(tape_position EQUAL tape_len)
      set(tape_contents "${tape_contents}0")
    endif()
  endif()
